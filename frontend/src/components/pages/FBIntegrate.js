...
const FBIntegrate = () => {
  const { getAuthUser, getFBData, getFBToken, removeFBData, getFBPageData } =
  ...
  useEffect(() => {
    if (window.location.search) {
      ...
      if (urlParams.code.length > 5) getFBData(urlParams.code, navigate);
    }
  }, []);
          ...
          {!!fbPageData && fbPageData?.name.length > 1 ? (
            <div>
              ...
              <h6 className="text-xl text-center">
                Integrated Page : {fbPageData?.name}
              </h6>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link
                to={backend_url + "facebook/login"}
                className="bg-[#004f97] p-4 w-96 text-white text-center text-lg rounded-md"
              >
                Connect Page
              </Link>
            </div>
