...
class AuthProvider extends React.PureComponent {
  ...
  getAuthUser = () => {
    const usr = this.state.authUser
      ? this.state.authUser
      : localStorage.getItem("authpegUser");
    if (!usr) return null;
  ...
  getFBPageData = () => {
    const usr = this.state.fbPageData
      ? this.state.fbPageData
      : localStorage.getItem("fbPegPageData");
    if (!usr) return null;
    ...
    API.fbLogin()
      ...
      .then((res) => {
        ...
        localStorage.setItem(
          "fbPegPageData",
          JSON.stringify(res.data?.pageData[0])
        );
        history("/fbIntegrate");
      });
  };
